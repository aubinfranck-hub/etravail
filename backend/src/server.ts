...
const app=express() as express.Application, port=Number(process.env.APP_PORT??3000), jwtSecret=process.env.JWT_SECRET??"development-only-change-me";
...
app.get("/api/v1/calendar",auth,async(req:Req,res)=>{try{res.json({data:await listCalendar(req.user!.role,req.user!.id,req.query.caseId?String(req.query.caseId):undefined)});}catch(e){handleError(res,e,"Calendrier indisponible");}});
