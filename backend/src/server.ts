...
router.post("/api/v1/cases/:id/hearings",auth,permission("hearing:manage"),async(req:Req,res:express.Response)=>{const a=await caseAccess(req,req.params.id);if(!a)return res.status(a===null?404:403).json({error:a===null?"Dossier introuvable":"Accès refusé"});try{if(!hearingStageAllowed(a.status))throw new Error("INVALID_WORKFLOW_STAGE");const h=await scheduleHearing({caseId:req.params.id,scheduledAt:req.body?.scheduledAt,room:req.body?.room});const c=await findCase(req.params.id);if(c){const body=`Une audience a été programmée pour le dossier ${c.reference} le ${new Date(h.scheduled_at).toLocaleString("fr-FR")}.`;await notify({userId:c.claimant_id,caseId:c.id,channel:"IN_APP",subject:"Audience programmée",body});if(c.assigned_to&&c.assigned_to!==c.claimant_id)await notify({userId:c.assigned_to,caseId:c.id,channel:"IN_APP",subject:"Audience programmée",body});}res.status(201).json({data:h});}catch(e){handleError(res,e,"Impossible de programmer l'audience");}});
