...
router.post("/api/v1/cases/:id/hearings",auth,permission("hearing:manage"),async(req:Req,res:express.Response)=>{try{const h=await scheduleHearing({caseId:req.params.id,scheduledAt:req.body?.scheduledAt,room:req.body?.room});const c=await findCase(req.params.id);if(c)await notify({userId:c.claimant_id,caseId:c.id,channel:"IN_APP",subject:"Audience programmée",body:`Une audience a été programmée pour le dossier ${c.reference} le ${new Date(h.scheduled_at).toLocaleString("fr-FR")}.`});res.status(201).json({data:h});}catch(e){handleError(res,e,"Impossible de programmer l'audience");}});
