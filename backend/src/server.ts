...
router.patch("/api/v1/conciliations/:id",auth,permission("hearing:manage"),async(req:Req,res:express.Response)=>{try{res.json({data:await updateConciliation(req.params.id,req.body?.status,req.body?.notes)});}catch(e){handleError(res,e,"Impossible de modifier la conciliation");}});
