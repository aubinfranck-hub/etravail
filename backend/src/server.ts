...
import {getNotifications,notify,notifyCaseParticipants,markNotificationRead} from "./services/notificationService.js";
...
// A worker thread (tesseract.js's OCR worker in particular) can emit an
// 'error' event outside of any request's promise chain; left unhandled,
// Node treats that as fatal and kills the whole process, taking every
// in-flight request down with it. Log and keep serving instead.
...
async function caseAccess(req:Req,id:string,write=false){const c=await findCase(id);if(!c)return null;if(req.user?.role==="CITOYEN"&&c.claimant_id!==req.user.id)return false;if(req.user?.role!=="CITOYEN"&&req.user?.role!=="ADMIN"&&c.assigned_to!==req.user?.id)return false;if(write&&!req.user?.role)return false;return c;}
...
router.post("/api/v1/cases/:id/parties",auth,async(req:Req,res:express.Response)=>{const a=await caseAccess(req,req.params.id,true);if(!a)return res.status(a===null?404:403).json({error:a===null?"Dossier introuvable":"Accès refusé"});if(req.user!.role!=="CITOYEN"&&!hasPermission(req.user!.role,"case:document"))return res.status(403).json({error:"Permission refusée"});if(!req.body?.type||!req.body?.fullName)return res.status(400).json({error:"type et fullName sont obligatoires"});try{await ensureCaseMutationAllowed(req.params.id,req.user!.role,"ADD_PARTY");res.status(201).json({data:await addParty({caseId:req.params.id,type:req.body.type,fullName:req.body.fullName,contact:req.body.contact})});}catch(e){handleError(res,e,"Impossible d'ajouter la partie");}});
...
router.post("/api/v1/cases/:id/hearings",auth,permission("hearing:manage"),async(req:Req,res:express.Response)=>{const a=await caseAccess(req,req.params.id);if(!a)return res.status(a===null?404:403).json({error:a===null?"Dossier introuvable":"Accès refusé"});try{if(!hearingStageAllowed(a.status))throw new Error("INVALID_WORKFLOW_STAGE");const h=await scheduleHearing({caseId:req.params.id,scheduledAt:req.body?.scheduledAt,room:req.body?.room});const c=await findCase(req.params.id);if(c){await notify({userId:c.claimant_id,caseId:c.id,channel:"IN_APP",subject:"Audience programmée",body:`Une audience a été programmée pour le dossier ${c.reference} le ${new Date(h.scheduled_at).toLocaleString("fr-FR")}.`});await notifyCaseParticipants({caseId:c.id,stage:"AUDIENCES",subject:"Audience programmée",body:`Une audience est programmée pour le dossier ${c.reference} le ${new Date(h.scheduled_at).toLocaleString("fr-FR")}.`});}res.status(201).json({data:h});}catch(e){handleError(res,e,"Impossible de programmer l'audience");}});
...
router.get("/api/v1/cases/:id/audit",auth,async(req:Req,res:express.Response)=>{const a=await caseAccess(req,req.params.id);if(!a)return res.status(a===null?404:403).json({error:a===null?"Dossier introuvable":"Accès refusé"});if(req.user?.role!=="CITOYEN"&&!hasPermission(req.user!.role,"case:read"))return res.status(403).json({error:"Permission refusée"});try{res.json({data:await listAudit(req.params.id)});}catch(e){handleError(res,e,"Journal d'audit indisponible");}});
