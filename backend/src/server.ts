...
import {getCases,openCase,transitionCase,assignCaseTo,getRequirements,ensureRequirementCanReceive,attachRequirementDocument,validateRequirement,getRequirementHistory,reassignCasesFromUser} from "./services/caseService.js";
...
router.post("/api/v1/admin/cases/:id/payment/setup",auth,permission("admin:manage"),async(req:Req,res:express.Response)=>{
 try{const a=await caseAccess(req,req.params.id,true);if(!a)return res.status(a===null?404:403).json({error:a===null?"Dossier introuvable":"Accès refusé"});
  const data=await setupPayment({caseId:req.params.id,feeAmount:Number(req.body?.feeAmount),currency:req.body?.currency,actorId:req.user!.id});res.json({data});
