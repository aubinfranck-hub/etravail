...
router.patch("/api/v1/conciliations/:id",auth,permission("hearing:manage"),async(req,res)=>{try{res.json({data:await updateConciliation(req.params.id,req.body?.status,req.body?.notes)});}catch(e){handleError(res,e,"Impossible de modifier la conciliation");}});
router.get("/api/v1/calendar",auth,async(req:Req,res)=>{try{res.json({data:await listCalendar(req.user!.role,req.user!.id,req.query.caseId?String(req.query.caseId):undefined)});}catch(e){handleError(res,e,"Calendrier indisponible");}});

router.get("/api/v1/notifications",auth,async(req:Req,res)=>{try{res.json({data:await getNotifications(req.user!.id)});}catch(e){handleError(res,e,"Impossible de récupérer les notifications");}});
...
router.get("/api/v1/cases/:id/audit",auth,permission("case:read"),async(req,res)=>{try{res.json({data:await listAudit(req.params.id)});}catch(e){handleError(res,e,"Journal d'audit indisponible");}});
router.get("/api/v1/cases/:id/decisions",auth,permission("case:read"),async(req,res)=>{try{res.json({data:await getDecisions(req.params.id)});}catch(e){handleError(res,e,"Impossible de récupérer les décisions");}});
...
router.get("/api/v1/search/documents",auth,permission("case:read"),async(req,res)=>{try{res.json({data:await searchInDocuments(String(req.query.q??""))});}catch(e){handleError(res,e,"Recherche impossible");}});
